...
def display_images_and_names(image_labels, name_labels, loc_labels, time_labels, more_labels, image_data):
    for i in range(min(len(image_labels), len(image_data) // 5)):
        ...
        name_labels[i].config(text="Name: " + name)
        loc_labels[i].config(text="Location: " + loc)
        time_labels[i].config(text="Timing: " + time)
        more_labels[i].config(text="More Info: " + more_info)
...
def create_suggestion(parent):
    ...
    suggestion_window = Toplevel(parent)
    suggestion_window.title("places")
    suggestion_window.configure(background="#111F4D")
    suggestion_window.resizable(False, False)

    input_frame = Frame(suggestion_window, bg='#111F4D')
    input_frame.grid(row=6, column=0, columnspan=3, padx=10, pady=10, sticky='n')

    image_frame = Frame(suggestion_window, bg='#111F4D')
    ...
    suggestion_window.geometry(f"{window_width}x{window_height}+{x_position}+{y_position}")
    ...
    accommodation_label = Label(suggestion_window, text="Search for places below", fg='#f7f7f7', bg='#111F4D',
    ...
    state_var = StringVar(suggestion_window)
    state_var.set('Select State')  # Default value
    state_menu = OptionMenu(suggestion_window, state_var, *cities_by_state.keys(), command=on_state_select)
    ...
    location_var = StringVar(suggestion_window)
    location_var.set('Select Location')  # Default value
    location_menu = OptionMenu(suggestion_window, location_var, 'Select Location')
    ...
    Back = Button(suggestion_window, text="Back", foreground='#f7f7f7', background='#D24545',
                  activeforeground='#D24545',
                  activebackground='#A94438', command=lambda: feature_back(suggestion_window, parent),
                  font=font_button)
    ...
    for i in range(3):
        ...
        loc_label = Label(image_frame, text="", cursor="hand2", wraplength=200)
        loc_label.grid(row=2, column=i, padx=10, pady=5)
        loc_labels.append(loc_label)

        time_label = Label(image_frame, text="", cursor="hand2", wraplength=200)
    ...
    Show = Button(suggestion_window, text="Display", foreground='#f7f7f7', background='#D24545',
    ...
    suggestion_window.grid_columnconfigure(0, weight=1, uniform="group1")
    suggestion_window.grid_columnconfigure(1, weight=1, uniform="group1")
    suggestion_window.grid_columnconfigure(2, weight=1, uniform="group1")

    suggestion_window.grid_rowconfigure(2, weight=0)
    suggestion_window.grid_rowconfigure(3, weight=0)
    suggestion_window.grid_rowconfigure(4, weight=0)
    suggestion_window.grid_rowconfigure(5, weight=0)
    suggestion_window.grid_rowconfigure(6, weight=0)
    suggestion_window.grid_rowconfigure(7, weight=0)


if __name__ == "__main__":
    window = tk.Tk()
    create_suggestion(window)
    window.mainloop()
